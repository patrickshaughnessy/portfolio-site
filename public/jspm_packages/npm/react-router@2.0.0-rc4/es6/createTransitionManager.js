...
function createTransitionManager(history, routes) {
  ...
  function isActive(location) {
    ...
    if (indexOnlyOrDeprecatedQuery && indexOnlyOrDeprecatedQuery !== true || deprecatedIndexOnly !== null) {
      process.env.NODE_ENV !== 'production' ? warning(false, '`isActive(pathname, query, indexOnly) is deprecated; use `isActive(location, indexOnly)` with a location descriptor instead: http://bit.ly/1VpTo9H') : undefined;
  ...
  function createLocationFromRedirectInfo(_ref) {
    ...
    return history.createLocation(history.createPath(pathname, query), state, REPLACE);
  }
