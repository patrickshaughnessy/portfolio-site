...
var Router = React.createClass({
  ...
  componentWillMount: function componentWillMount() {
    ...
    process.env.NODE_ENV !== 'production' ? warning(!(parseQueryString || stringifyQuery), '`parseQueryString` and `stringifyQuery` are deprecated. Please create a custom history. See http://bit.ly/1NRMoPX for details.') : undefined;
  ...
  wrapDeprecatedHistory: function wrapDeprecatedHistory(history) {
    ...
    if (history) {
      ...
      process.env.NODE_ENV !== 'production' ? warning(false, '`Router` no longer defaults the history prop to hash history. Please use the `hashHistory` singleton instead. http://bit.ly/1ktRibg') : undefined;
